...
The background uses a small WebGL mesh, capped at 960 pixels wide. It draws on layout or pointer changes only; there is no continuous animation loop. Reduced motion disables pointer response. A CSS mesh remains underneath for unavailable or lost WebGL contexts. The app window uses one backdrop blur layer, visible through only the sidebars and toolbar. The terminal body (including composer and footer) is opaque. Reduced transparency uses opaque surfaces throughout. Desktop windows use a 16:10 aspect ratio, a 248px sidebar from the native app, and a 320px Changes pane.
