...
The desktop consumes observed modes whenever available. When an older Holder or restored checkpoint has no keyboard metadata, arrows and Home/End retain the historical normal-cursor CSI encoding (including modified navigation) so upgrading does not disable basic editing in surviving sessions. This desktop-only compatibility policy does not invent or persist observed modes; known application-cursor modes still produce SS3 sequences. Numeric-keypad input still reports unavailable modes, and the automated key API remains strict. The Engine continues to reject disconnected, revoked, or incompatible enhanced-keyboard input. The patched GPUI macOS adapter provides callback-scoped hardware identity without changing logical text or shortcuts. The terminal uses it to distinguish physical numeric-keypad keys from the top row and Return, then invokes the same encoder as the CLI. Saved or synthetic GPUI events outside native dispatch do not inherit hardware identity.

Command–Left and Command–Right send the readline line-start/line-end controls (Ctrl+A/Ctrl+E). Command–Backspace keeps its existing Ctrl+U mapping. Other Command shortcuts remain owned by the app. Selecting an editable prompt is a child-application feature: Command+A is not translated to Ctrl+A, which moves the cursor rather than selecting text.
