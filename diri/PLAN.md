...
- Data channel: first line is NDJSON `AttachRequest{attach: <sessionID>, fromOffset?, token?, role}`, then binary frames both ways: `[type u8][len u32 BE][payload]`, max 16 MiB. Types: `input=2`, `resize=3` (cols u16 + rows u16 BE), `ping=6`/`pong=7`, `grid=8`, `scroll=9` (dir u8, lines u16, col u16, row u16 BE), `modes=10` (byte 0: bit0 altScreen, bit1 historical any-mouse compatibility, bits2–3 mouse tracking off/1000/1002/1003, bit4 legacy/SGR encoding, bit5 bracketed paste, bit6 DECCKM/application cursor keys, bit7 DECCKM value-known; optional bytes 1–8: opaque acknowledgement token), `mouse=11` (pre-encoded report bytes on the raw interactive path). A current desktop returns a token in a client-to-daemon Modes frame only after GPUI applies that snapshot; the daemon then admits input for that session generation. Historical daemons ignore the additive acknowledgement and historical clients ignore the trailing token. Types 1/4/5 are legacy byte-replay — ignore.
