...
- Event resume: track max `seq`; on (re)subscribe send `events.subscribe{sinceSeq}` — the daemon replays from a 4096-event ring. If either the daemon subscription or the client's local broadcast reports a hole, pause incremental projection, replace it with `session.list`, then discard replay through that response's additive `eventSeq` watermark. Older daemons omit `eventSeq`; capture the conservative pre-request receive watermark instead.
