...
use gpui::{
    AnyWindowHandle, BackgroundExecutor, Bounds, Capslock, CursorStyle, DevicePixels,
...
fn drawable_size_for_visibility(
    visible: bool,
    full_size: Size<DevicePixels>,
) -> Size<DevicePixels> {
    if visible {
        full_size
    } else {
...
impl MacWindowState {
    ...
    fn update_backing_surface(&mut self) {
        let full = self.content_size().to_device_pixels(self.scale_factor());
        self.renderer
            .update_drawable_size(drawable_size_for_visibility(self.is_visible(), full));
        if !self.is_visible() {
            self.renderer.release_window_sized_textures();
        }
    }
...
fn update_window_scale_factor(window_state: &Arc<Mutex<MacWindowState>>) {
    ...
    let visible = lock.is_visible();
    lock.renderer
        .update_drawable_size(drawable_size_for_visibility(visible, drawable_size));
...
mod tests {
    ...
    #[test]
    fn occluded_windows_release_their_retina_backing_surface() {
        let full = size(DevicePixels(4_112), DevicePixels(2_580));
        assert_eq!(
            drawable_size_for_visibility(false, full),
            size(DevicePixels(1), DevicePixels(1))
        );
        assert_eq!(drawable_size_for_visibility(true, full), full);
    }
}
