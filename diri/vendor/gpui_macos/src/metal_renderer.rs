...
/// Terminal/UI scenes are cheap to encode and do not benefit from queuing a
/// third full-window Retina surface. Two preserves CPU/GPU overlap while
/// removing one drawable's IOSurface residency.
const MAXIMUM_DRAWABLE_COUNT: u64 = 2;
...
mod drawable_tests {
    ...
    #[test]
    fn interactive_windows_keep_two_frames_in_flight() {
        assert_eq!(MAXIMUM_DRAWABLE_COUNT, 2);
    }
}

#[cfg(test)]
#[path = "metal_renderer_tests.rs"]
...
impl MetalRenderer {
    ...
    pub fn new(instance_buffer_pool: Arc<Mutex<InstanceBufferPool>>, transparent: bool) -> Self {
        ...
        layer.set_maximum_drawable_count(MAXIMUM_DRAWABLE_COUNT);
    ...
    /// Drop full-window transient textures when AppKit reports the window fully
    /// occluded. The CAMetalLayer is resized by the caller; these renderer-owned
    /// path targets otherwise retain their previous dimensions indefinitely.
    ...
    fn draw_primitives_to_texture(
        ...
        // GPUI's path rasterizer needs a full-window resolve texture and, on
        // supported hardware, a 4x MSAA target. Most scenes contain only
        // quads and glyph sprites, so allocating both targets at every window
        // resize needlessly charges tens or hundreds of MiB to the process.
        // Preserve full-quality path rendering, but pay for it only in frames
        // that actually contain a path.
...
fn build_pipeline_state(
    ...
    // Preserve source-over coverage for WindowServer's backdrop compositing.
    // Adding alpha makes antialiased text and badge edges occlude too much of
    // a bright desktop, producing dark fringes in transparent windows.
...
fn build_path_sprite_pipeline_state(
    ...
    color_attachment.set_source_alpha_blend_factor(metal::MTLBlendFactor::One);
    color_attachment.set_destination_rgb_blend_factor(metal::MTLBlendFactor::OneMinusSourceAlpha);
    // Path sprites already carry premultiplied RGB, but their coverage still
    // combines with the destination using the same source-over alpha rule.
    color_attachment.set_destination_alpha_blend_factor(metal::MTLBlendFactor::OneMinusSourceAlpha);
