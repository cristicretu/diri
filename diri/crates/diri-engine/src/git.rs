...
const DIFF_RESPONSE_MARKER: &[u8] = b"DIRI_GIT_V1\0";
...
printf 'DIRI_GIT_V1\0%s\0%s\0' "$root" "$base_ref"
(
...
fn parse_working_diff(stdout: Vec<u8>) -> std::io::Result<diri_proto::SessionReadDiffResult> {
    let marker = stdout
